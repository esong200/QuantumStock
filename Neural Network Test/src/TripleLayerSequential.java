...
public class TripleLayerSequential {
	public static void main(String[] args) {
		String comp = "AAPL";
		String runName = "UltraBigSequential";
		...
		for(int i=(data.size()*3)/4; i>0; i--) {
			data.remove(i);
		}
		for(int i=(pseudorandomData.size()*3)/4; i>0; i--) {
			pseudorandomData.remove(i);
		}
		for(int i=(ans.size()*3)/4; i>0; i--) {
			ans.remove(i);
		}
		for(int i=(pseudorandomAns.size()*3)/4; i>0; i--) {
			pseudorandomAns.remove(i);
		}
