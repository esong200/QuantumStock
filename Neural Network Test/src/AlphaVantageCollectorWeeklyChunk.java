...
public class AlphaVantageCollectorWeeklyChunk extends CSVReadWrite{
			...
			List<StockData> stockData = response.getStockData();
				stockData.forEach(stock -> {
					double[] inputData = new double[5];
						...
						intermediateData.add(inputData);		
							
				});
			intermediateData.remove(intermediateData.size()-1);
			for(int i=0; i<intermediateData.size()/4; i++) {
				double[][] intermediateDataArr = new double[4][5];
				for(int j=0; j<4; j++) {
					intermediateDataIndex++;
					intermediateDataArr[j] =  intermediateData.get(intermediateDataIndex);
				}
				data.add(intermediateDataArr);
			}
	...
	ADOSC adosc = technicalIndicators.adosc(symbol, Interval.WEEKLY, FastPeriod.of(12), SlowPeriod.of(26));
		...
		intermediateData.remove(0);
		ArrayLength =  intermediateData.size();
		for(int i=0; i<intermediateData.size()/4; i++) {
			double[][] intermediateDataArr = new double[4][5];
			for(int j=0; j<4; j++) {
				intermediateDataIndex++;
	...
	public static ArrayList<double[]> answerCompile (String symbol){
		...
		for(int i = 0; i < weekly.size()-1;i++){
			...
			double percentChange = ((weekly.get(i)[3][0] - weekly.get(i+1)[0][0])/weekly.get(i+1)[0][0])*100;
